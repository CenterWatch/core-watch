...
function buscarTempoRealVolume(uuid) {
    ...
    var instrucaoSql = `select r.*, v.volume_total from registro_volume r join sessao s on r.fk_sessao = s.id_sessao join volume v on r.fk_volume = v.uuid join maquina m on m.id_maquina = s.fk_maquina where uuid = "${uuid}" order by dt_hora desc limit 1;`;
